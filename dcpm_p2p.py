...
class P2PNode:
    def __init__(self, host, port, public_key_pem, private_key_pem, tls_cert=None, tls_key=None):
        self.host = host
        self.port = port
    ...
    async def handle_dnscrypt_stream(self, stream):
        ...
        if self.check_traffic_limit(peer_id):
            ...
            self.node.addStreamHandler(dnscrypt_stream, self.handle_dnscrypt_stream_data)
        else:
            stream.reset()
    ...
    async def old_download_file(self, file_id, block_hashes):
        """从DHT下载文件,支持断点续传"""
        file_data = b""
    ...
    async def old_download_package(self, package_id):
        """从网络下载软件包"""
        package_data_str = await self.dht.get(package_id)
        if package_data_str:
            package_data = eval(package_data_str)
    ...
    async def old_download_package_content(self, package_data):
        """下载软件包内容"""
        package_content = b""
        block_ids = []
