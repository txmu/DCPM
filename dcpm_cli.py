...
def parse_args():
   ...
   parser.add_argument("--dep-mode", choices=["none", "suggest", "auto"], default="none", help="依赖解决模式: none - 不解决依赖; suggest - 输出依赖解决建议; auto - 自动解决依赖")
       ...
       if args.dep_mode == "suggest":
           ...
           for software_id, package_versions in resolved.items():
               for package_version in package_versions:
                   print(f"{package_version.name} ({package_version.version_id}) - [{package_version.software_id}, {package_version.package_id}, {package_version.version_id}]")
